...
export default class AddReport extends React.Component {
    ...
    updateFormField = (evt) => {
        this.setState(
            ...
            () => {
                let textValid =
                    this.state.reportId.length >= 1 &&
                        this.state.reportTitle.length >= 1 &&
                        this.state.reportContent.length >= 1 &&
                        this.state.reportReferences.length >= 1
                        ? true
                        : false;
    ...
    updateitems = (evt) => {
        if (this.state.reportTags.includes(evt.target.value)) {
            ...
            this.setState(
                ...
                () => {
                    let tagsValid =
                        this.state.reportTags.includes(this.getDate()) &&
                            this.state.reportTags.includes(
                                "Refer to peer reviewed articles above for integrity of report"
                            )
                            ? true
                            : false;
                    ...
                    <div className="form-label checkbox">
                        <p>
                            <input
                                ...
                                checked={this.state.reportTags.includes(this.getDate())}
                            />
                            <label>&nbsp;Published Date(Current Date)</label>
                        </p>
                        ...
                        <p>
                            <input
                                type="checkbox"
                                name="reportTags"
                                value="Refer to peer reviewed articles above for integrity of report"
                                onChange={this.updateitems}
                                checked={this.state.reportTags.includes(
                                    "Refer to peer reviewed articles above for integrity of report"
                                )}
                            />
                            <label>&nbsp;I have included References</label>
                        </p>
                    </div>
                </div>
